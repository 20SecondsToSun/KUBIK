...
namespace kubik
{
	...
	public:
		...
		void draw() override
		{
			...
			textTools().textFieldDraw("‘Œ“Œ√–¿‘»–”≈Ã", &font, Color::white(), Vec2f(100.0f, 100.0f));
			textTools().textFieldDraw(" ŒÀ»◊≈—“¬Œ —Õ»Ã Œ¬ " + to_string(shotsNum), &font, Color::white(), Vec2f(100.0f, 200.0f));
			textTools().textFieldDraw("¬–≈Ãﬂ ŒƒÕŒ√Œ —Õ»Ã ¿(—≈ .) " + to_string(secBetweenShots), &font, Color::white(), Vec2f(100.0f, 300.0f));
		}
