...
namespace kubik
{
	...
	public:
		...
		void draw() override
		{
			...
			textTools().textFieldDraw("‘Œ“Œ√–¿‘»–”≈Ã", &font, Vec2f(100.0f, 100.0f), Color::white());
			textTools().textFieldDraw(" ŒÀ»◊≈—“¬Œ —Õ»Ã Œ¬ " + to_string(shotsNum), &font, Vec2f(100.0f, 200.0f), Color::white());
			textTools().textFieldDraw("¬–≈Ãﬂ ŒƒÕŒ√Œ —Õ»Ã ¿(—≈ .) " + to_string(secBetweenShots), &font, Vec2f(100.0f, 300.0f), Color::white());
		}
