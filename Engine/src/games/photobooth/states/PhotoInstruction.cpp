...
PhotoInstruction::PhotoInstruction(PhotoboothSettingsRef settings): animTime(0.8f)
{
...
void PhotoInstruction::reset(PhotoboothSettingsRef set)
{
	...
	titleTexPos = Vec2f(0.5 * (getWindowWidth() - titleTex.getWidth()), titlePositionY - titleTex.getHeight() * 0.5f);				
}
