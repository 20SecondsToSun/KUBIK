...
*
*--*/
bool twitCurl::statusUpdateWithMedia( std::string& newStatus, std::string inReplyToStatusId )
{
    if( newStatus.empty() )
    {
        return false;
...
bool  twitCurl::uploadPictureFromFile(std::vector<std::string> filelinks, std::string& newStatus)
{
	...
	for (size_t i = 0; i < len-1; i++)
	{
	 ...
	 forms[len-1].option  = CURLFORM_END;

	//  CURLFORM_CONTENTTYPE, "image/jpg",

	 curl_formadd(&post, &last, CURLFORM_COPYNAME, "media[]",
               CURLFORM_ARRAY, forms, CURLFORM_END);	
	curl_formadd( &post, &last, CURLFORM_COPYNAME, "status", CURLFORM_COPYCONTENTS, newStatus.c_str(), CURLFORM_END );
...
bool  twitCurl::getTwitterConfiguration()
{
	return performGet( twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
                       twitterDefaults::TWITCURL_CONFIGURATION_URL +
                       twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType] );
}
...
*
*--*/
bool twitCurl::performPostWithMedia( const std::string& postUrl, std::string dataStr )
{
    /* Return if cURL is not initialized */
    if( !isCurlInit() )
    {
...
static int my_trace(CURL *handle, curl_infotype type, char *data, size_t size, void *userp)
{
	...
	dump(text, pFile, (unsigned char *)data, size);
	return 0;
}
...
bool twitCurl::performPost( const std::string& postUrl, std::string dataStr, bool headerInBody, const std::string& additional_post_params)
{
	...
	curl_easy_setopt(m_curlHandle, CURLOPT_SSL_VERIFYPEER, 0L); // this line makes it work under https
...
bool twitCurl::oAuthRequestToken( std::string& authorizeUrl /* out */, const std::string& callback_url )
{
                                ...
                                oAuthHeader, false ) )
    {
		...
		auto pairs = m_oAuth.getRawKeyPairs(eOAuthHttpPost,
			twitCurlDefaults::TWITCURL_PROTOCOLS[m_eProtocolType] +
			oAuthTwitterApiUrls::OAUTHLIB_TWITTER_REQUEST_TOKEN_URL,
			std::string(""),
			oAuthHeader, false);
...
bool twitCurl::oAuthHandlePIN( const std::string& authorizeUrl /* in */ )
{
    ...
    if( CURLE_OK == curl_easy_perform( m_curlHandle ) )
    {
        ...
        {			
			...
			ci::app::console() << "oauthTokenVal :::::::::::::::::: " << oauthTokenVal << std::endl;
        }
    }
